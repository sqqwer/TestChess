...
std::vector<DoublePair>		Kings::GetPossitionToGo(
...
bool Kings::FindTurn(std::vector<DoublePair>& ObjectVector, DoublePair i)
{
	bool EBV;
	bool Side = this->Side;
	int h = 30, w = 30;
...
std::vector<DoublePair> Kings::CanAttack()
{
	std::vector<DoublePair> Buffer;
...
std::vector<DoublePair> Kings::CanMove(DoublePair Map[8][8], DoublePair Poss)
{
	std::vector<DoublePair> Buffer;
	...
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
		{
			if (((PossitonI - i <= 1 && PossitonJ - j  <= 1) &&
				(PossitonI - i >= -1 && PossitonJ - j >= -1) ) && 
